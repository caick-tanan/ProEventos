        ...
        public async Task<PageList<Palestrante>> GetAllPalestrantesAsync(PageParams pageParams,bool includeEventos = false)
        {
           IQueryable<Palestrante> query = _context.Palestrantes
            ...
            query = query.AsNoTracking().Where(p => (p.MiniCurriculo.ToLower().Contains(pageParams.Term.ToLower()) ||
                                                     ...
                                                     p.User.Funcao == Domain.Enum.Funcao.Palestrante) //vai retornar apenas os usuários que tenha como função -> palestrantes, dessa maneira não precisará de deleção do palestrante
                                        .OrderBy(p => p.Id); // oredena por ID

            return await PageList<Palestrante>.CreateAsync(query, pageParams.PageNumber, pageParams.pageSize); // retorna o array 
        }
